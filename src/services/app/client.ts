...
import { calculateBackoffDelay, DEFAULT_RETRY_LIMIT, isRetryableClientError, type RetryOptions } from "./retry.js";
...
export class Client {
  ...
  subscribe<Subscription extends GraphQLSubscription>(
    ...
    const maxRetries = retryOptions?.maxAttempts ?? DEFAULT_RETRY_LIMIT;
    let retryCount = 0;
    ...
    const scheduleRetry = (error: ClientError, logError: unknown): boolean => {
      if (!retryOptions || !isRetryableClientError(error) || retryCount >= maxRetries) {
        return false;
      }
