...
import { calculateBackoffDelay, DEFAULT_RETRY_LIMIT, isRetryableClientError } from "../../src/services/app/retry.js";
...
export const makeMockEditSubscriptions = (): MockEditSubscriptions => {
  ...
  mock(Client.prototype, "subscribe", (_ctx, options) => {
    ...
    const scheduleRetry = (error: ClientError): boolean => {
      if (!options.retry || !isRetryableClientError(error) || retryCount >= maxRetries) {
        return false;
      }
