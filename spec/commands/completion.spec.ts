...
import { completeApp, completeEnvironment } from "../../src/services/app/app.js";
...
import { completePreference } from "../../src/services/filesync/strategy.js";
...
describe("completion", () => {
  ...
  describe("providers", () => {
    describe("completePreference", () => {
      it("returns all values with empty partial", async () => {
        const result = await completePreference(testCtx, "", []);
        expect(result).toEqual(["local", "environment"]);
      });

      it("filters by prefix", async () => {
        const result = await completePreference(testCtx, "l", []);
        expect(result).toEqual(["local"]);
      });
    });

    describe("completeApp", () => {
      beforeEach(() => {
        loginTestUser();
        nockTestApps();
      });
      ...
      it("returns all app slugs with empty partial", async () => {
        const result = await completeApp(testCtx, "", []);
        expect(result).toEqual(["test", "test2"]);
      });

      it("filters by prefix", async () => {
        const result = await completeApp(testCtx, "test2", []);
        expect(result).toEqual(["test2"]);
      });
    });

    describe("completeApp without auth", () => {
    ...
    describe("completeEnvironment", () => {
      ...
      it("returns environment names for a specific app", async () => {
        const result = await completeEnvironment(testCtx, "", ["--app", "test"]);
        expect(result).toEqual(testApp.environments.map((e) => e.name));
      });

      it("filters by prefix", async () => {
        const result = await completeEnvironment(testCtx, "dev", ["--app", "test"]);
        expect(result).toEqual(["development"]);
      });

      it("returns deduplicated env names when no --app given", async () => {
        const result = await completeEnvironment(testCtx, "", []);
    ...
    describe("completeEnvironment without auth", () => {
      it("returns empty when not logged in", async () => {
        const result = await completeEnvironment(testCtx, "", ["--app", "test"]);
        expect(result).toEqual([]);
      });
