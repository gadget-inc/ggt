...
export const subcommandDefs: readonly SubcommandDef[] = [
  { name: "list", description: "List all environment variables" },
  { name: "get", description: "Get the value of an environment variable" },
  { name: "set", description: "Set one or more environment variables", args: setArgs },
  { name: "delete", description: "Delete one or more environment variables", args: deleteArgs },
  { name: "import", description: "Import environment variables from another environment or file", args: importArgs },
];
...
export const run: Run<typeof args> = async (ctx, args) => {
  ...
  if (args._.includes("-h") || args._.includes("--help")) {
    if (subcommand) {
      ...
      if (def) {
        println(
          renderShortUsage("var " + subcommand, { description: def.description, args: { ...AppIdentityArgs, ...def.args }, examples: [] }),
        );
        process.exit(0);
      }
    }
