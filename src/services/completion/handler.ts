 ...
 */
const findFirstPositional = (tokens: string[], args: ArgsDefinition, from: number): { value: string; index: number } | undefined => {
  let skipNext = false;
...
const getCompletionCandidates = async (ctx: Context, tokens: string[]): Promise<string[]> => {
  ...
  const commandMatch = findFirstPositional(preceding, rootArgs, 0);
  const commandName = commandMatch?.value;
  ...
  if (lastPreceding?.startsWith("-") && !lastPreceding.includes("=")) {
    const result = await tryCompleteFlag(ctx, lastPreceding, partial, tokens, mergedArgs, subcommandName, mod);
    if (result !== undefined) {
      return result;
    }
  }
  ...
  if ("subcommands" in mod && mod.subcommands && !subcommandName) {
    ...
    const flagNames = getFlagNames(mergedArgs, partial);
