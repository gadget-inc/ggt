...
export const run: Run<DebuggerArgs> = async (ctx, args) => {
  ...
  const spinner = spin(StartingMessage);
  ctx.log.info("debugger command started");

  const syncJson = await SyncJson.loadOrInit(ctx, { command: "debugger", args, directory });
...
class RemoteDebuggerConnection {
  ...
  async connect(): Promise<WebSocket> {
    ...
    const ws = new WebSocket(`wss://${this._subdomain}.${config.domains.app}/edit/api/debugger`, {
      headers: { ...this._authHeaders, "x-gadget-debugger-session-id": this._sessionId },
    });
    ...
    const task: VSCodeTask = {
      ...
      problemMatcher: {
        ...
        background: {
          ...
          endsPattern: "ggt debugger running on ws",
        },
      },
    };
